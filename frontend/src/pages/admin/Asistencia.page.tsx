...
import {
    attendanceServices, AttendanceEntity
...
const AsistenciaPage: React.FC = () => {
    const [records, setRecords] = useState<AttendanceEntity[]>([]);
    ...
    useEffect(() => {
        const fetchRecords = async () => {
            const data = await attendanceServices.getAll();
            setRecords(data);
    ...
    useEffect(() => {
        ...
        const filtered = records.filter(
            (record) =>
                ...
                record..toLowerCase().includes(lowerSearch),
        );
    ...
    const handleExportCSV = () => {
        exportToCSV(
            filteredRecords,
            `asistencia_${new Date().toISOString().slice(0, 19)}`,
        );
    };
